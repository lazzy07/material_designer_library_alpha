    ...
    namespace OPENCL{
      ...
      std::vector<Ref<MATD::ENGINE::Device>> Platform::GetCompatibleDevices(){
        ...
        for (auto device : m_Devices) {
          cl_int error = 0;
          auto name = device.getInfo<CL_DEVICE_NAME>(&error);
          auto deviceType = device.getInfo<CL_DEVICE_TYPE>(&error);
          auto vendorID = device.getInfo<CL_DEVICE_VENDOR_ID>(&error);
          auto deviceMaxComputeUnits = device.getInfo<CL_DEVICE_MAX_COMPUTE_UNITS>(&error);
          auto maxWorkItemDimensions = device.getInfo<CL_DEVICE_MAX_WORK_ITEM_DIMENSIONS>(&error);
          auto deviceMaxWorkItemSizes = device.getInfo<CL_DEVICE_MAX_WORK_ITEM_SIZES>(&error);
          auto deviceMaxGroupSize = device.getInfo<CL_DEVICE_MAX_WORK_GROUP_SIZE>(&error);
